...
namespace Software_Test_App.Tests
{
    public class SystemTests : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        // Senaryo 1: Kullanıcı Kaydı → Entry Oluşturma → Review Ekleme → Tag Ekleme
        // Bu test gerçek bir kullanıcının sisteme girip içerik oluşturma sürecini simüle eder
        [Fact]
        public async Task CompleteUserJourney_CreateUser_AddEntry_AddReview_AddTag_SystemTest()
        {
            ...
            // 1. Adım: Yeni kullanıcı kaydı oluştur
            ...
            // 2. Adım: Kullanıcı bir entry (gönderi) oluşturur
            ...
            // 3. Adım: Entry için bir değerlendirme (review) ekle
            ...
            // 4. Adım: Entry'ye ilgili tagları ekle
            ...
            // 5. Adım: Oluşturulan entry'yi tekrar getir ve tüm verilerin doğru olduğunu kontrol et
        ...
        // Senaryo 2: İçerik Listeleme → Detay Görüntüleme → Güncelleme → Silme
        // Bir kullanıcının mevcut içerikleri yönetme sürecini test eder
        [Fact]
        public async Task ContentManagement_List_View_Update_Delete_SystemTest()
        {
            ...
            // 2. Adım: Spesifik bir içeriğin detayını görüntüle
        ...
        // Senaryo 3: Arama İşlemi → İçerik Oluşturma → Arama Yapma → Sonuçları Görüntüleme
        // Kullanıcının arama fonksiyonunu kullanma senaryosunu test eder
        [Fact]
        public async Task SearchWorkflow_CreateEntries_PerformSearch_ViewResults_SystemTest()
        {
            ...
            SeedDatabase(context =>
            {
                ...
                // Farklı konularda birden fazla entry oluştur
                context.Entries.Add(new Entry 
                { 
                    Id = 10201, 
        ...
        // Senaryo 4: Çoklu Kullanıcı Etkileşimi → Farklı Kullanıcılar → Birden Fazla Entry → Çapraz Review'lar
        ...
        public async Task MultiUserInteraction_MultipleUsers_CrossReviews_SystemTest()
        {
            ...
            // 1. Adım: İlk kullanıcıyı oluştur ve entry ekle
        ...
        // Senaryo 5: Kompleks İş Akışı → Entry Oluştur → Çoklu Review → Tag Ekleme → Güncelleme → Bazı Review'ları Sil
        // Gerçek dünya senaryosunda karmaşık iş akışlarını test eder
        [Fact]
        public async Task ComplexBusinessScenario_FullWorkflow_WithModifications_SystemTest()
        {
            ...
            // 3. Adım: Birden fazla kullanıcıdan review al
            ...
            // 5. Adım: Makaleyi güncelle (içerik geliştirme)
            ...
            // 6. Adım: Düşük puanlı review'lardan birini sil (moderasyon)
            ...
            // 7. Adım: Son durumu kontrol et - Entry hala mevcut olmalı
            ...
            // 8. Adım: Kalan review'ları kontrol et
            ...
            // Silinen review (10404) artık listede olmamalı
            Assert.DoesNotContain(remainingReviews, r => r.Id == 10404);
        }

        // Senaryo 6: Hata Yönetimi → Geçersiz Verilerle İşlemler → Doğru Hata Kodları
        // Sistemin hata durumlarını doğru şekilde ele aldığını test eder
        [Fact]
        public async Task ErrorHandling_InvalidOperations_ProperErrorCodes_SystemTest()
        {
            ...
            // 1. Adım: Var olmayan bir entry'ye review eklemeye çalış
            var invalidReview = new Review
            {
                Id = 10501,
                Text = "Bu review geçersiz bir entry için",
                Rating = 5,
                EntryId = 99999 // Var olmayan entry ID
            };
            ...
            // 2. Adım: Var olmayan bir kullanıcı ile entry oluşturmaya çalış
            var invalidEntry = new Entry
            {
                Id = 10501,
                Title = "Geçersiz Entry",
                Content = "Bu entry geçersiz bir kullanıcı için",
                UserId = 99999 // Var olmayan user ID
            };
            ...
            // 3. Adım: Var olmayan bir kaynağı getirmeye çalış
            var notFoundResponse = await client.GetAsync("/api/Entries/99999");
            Assert.Equal(HttpStatusCode.NotFound, notFoundResponse.StatusCode);

            // 4. Adım: Var olmayan bir tag'i güncellemeye çalış
            ...
            // 5. Adım: Var olmayan bir kaynağı silmeye çalış
        ...
        // Senaryo 7: Toplu Veri İşleme → Çoklu Kaynak Oluşturma → Listeleme → Filtreleme
        // Sistemin çoklu kaynak işlemlerini doğru şekilde yönettiğini test eder
        [Fact]
        public async Task BulkDataProcessing_CreateMultiple_ListAndFilter_SystemTest()
        {
            ...
            // 1. Adım: Birden fazla kullanıcı oluştur
            ...
            // 2. Adım: Her kullanıcı için birden fazla entry oluştur
            ...
            Assert.True(allEntries.Count >= 6); // En az 6 entry olmalı

            // 4. Adım: Her entry'ye tag ekle
            ...
            // 5. Adım: Tüm tagları getir ve kontrol et
        ...
        // Senaryo 8: Veri Bütünlüğü → İlişkili Kaynaklarla İşlemler → Tutarlılık Kontrolü
        // İlişkili verilerin bütünlüğünü ve tutarlılığını test eder
        [Fact]
        public async Task DataIntegrity_RelatedResources_ConsistencyCheck_SystemTest()
        {
            ...
            // 1. Adım: Temel veri yapısını oluştur
            ...
            // 2. Adım: İlişkili review'lar ekle
            ...
            // 4. Adım: Entry'yi getir ve ilişkilerin var olduğunu doğrula
            ...
            // 5. Adım: Review'ları getir ve entry ile ilişkili olduklarını kontrol et
            ...
            // 6. Adım: Tagları getir ve entry ile ilişkili olduklarını kontrol et
            ...
            // 7. Adım: Entry'yi güncelle ve ilişkilerin korunduğunu doğrula
            ...
            // 8. Adım: Güncellemeden sonra ilişkilerin hala geçerli olduğunu kontrol et
